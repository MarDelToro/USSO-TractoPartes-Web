...
(() => {
  ...
  function load() {
    ...
    // Published bundles can't fetch a project sidecar file — the bundler
    // inlines it as a blob URL under window.__resources instead (see the
    // ext-resource-dependency meta tag in each page that uses this file).
    const resolved = (window.__resources && window.__resources['image-slots-state']) || STATE_FILE;
    loadP = fetch(resolved)
  ...
  const stylesheet =
    ...
    // touch-action:none only while actively reframing (dragging/pinching to
    // pan-zoom) — unconditionally disabling it breaks native touch-swipe on
    // any ancestor horizontal scroller/carousel the slot sits inside.
